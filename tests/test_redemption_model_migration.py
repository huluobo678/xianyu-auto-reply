...
class RedemptionModelMigrationTests(unittest.TestCase):
    ...
    def test_ai_unlimited_field_present_on_three_models(self):
        for model in (BillingPlan, AIQuotaPackage, AIQuotaGrant):
            ...
            self.assertFalse(model.__table__.columns['ai_unlimited'].nullable is False
    ...
    def test_user_subscription_pending_downgrade_fields(self):
        ...
        expected = {
            'pending_plan_id', 'pending_plan_code', 'pending_billing_cycle',
            'pending_duration_months', 'pending_account_limit',
            'pending_monthly_ai_quota', 'pending_ai_unlimited',
            'pending_feature_snapshot', 'pending_source',
        }
    ...
    def test_redemption_code_digest_unique_constraint(self):
        constraints = {c.name for c in RedemptionCode.__table__.constraints
                       if c.__class__.__name__ == 'UniqueConstraint'}
        self.assertIn('uk_redemption_code_digest', constraints)

    def test_redemption_record_idempotency_unique(self):
        constraints = {c.name for c in RedemptionRecord.__table__.constraints
                       if c.__class__.__name__ == 'UniqueConstraint'}
