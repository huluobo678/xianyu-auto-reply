...
from sqlalchemy import BigInteger, Date, DateTime, Index, Integer, JSON, Numeric, String, UniqueConstraint, func
...
class BillingPlan(RecordMixin, Base):
    ...
    monthly_ai_quota: Mapped[int] = mapped_column(BigInteger, nullable=False, server_default='0')
...
class BillingPlanPrice(RecordMixin, Base):
    ...
    __table_args__ = (UniqueConstraint('plan_id', 'billing_cycle', name='uk_billing_plan_cycle'),)
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
...
class AIQuotaPackage(RecordMixin, Base):
    ...
    bonus_quota: Mapped[int] = mapped_column(BigInteger, nullable=False, server_default='0')
    ai_unlimited: Mapped[bool] = mapped_column(nullable=False, server_default='0')
    amount: Mapped[Decimal] = mapped_column(Numeric(10, 2), nullable=False)
...
class BillingOrder(RecordMixin, Base):
    ...
    entitlement_status: Mapped[str] = mapped_column(String(24), nullable=False, server_default='pending')
    entitlement_attempts: Mapped[int] = mapped_column(Integer, nullable=False, server_default='0')
...
class UserSubscription(RecordMixin, Base):
    ...
    pending_ai_unlimited: Mapped[bool] = mapped_column(nullable=False, server_default='0')
