...
class BillingService:
    ...
    async def list_catalog(self) -> dict:
        plans = list(await self.session.scalars(
            select(BillingPlan).where(BillingPlan.enabled.is_(True)).order_by(BillingPlan.sort_order)
        ))
        prices = list(await self.session.scalars(
            select(BillingPlanPrice).where(BillingPlanPrice.enabled.is_(True))
        ))
        addons = list(await self.session.scalars(
            select(AIQuotaPackage).where(AIQuotaPackage.enabled.is_(True)).order_by(AIQuotaPackage.sort_order)
        ))
    ...
    async def create_order(
        ...
        existing = await self.session.scalar(select(BillingOrder).where(
        ...
        order = BillingOrder(
            order_no=self._new_order_no(), user_id=user_id,
            request_key=request_key, product_type=product_type,
            product_id=product.id, product_code=code, product_name=name,
            product_snapshot=snapshot, amount=product.amount,
        )
